<template>
  <div id="ag-canvas">
    <div class="ag-btn-group">
      <span @click="handleExit" class="ag-btn exitBtn" :class="{ disabled: !readyState }" title="Exit">
        <i class="ri-logout-box-line ag-icon ag-icon-leave"></i>
      </span>
      <span v-if="attendeeMode === 'video'" @click="handleCamera" class="ag-btn videoControlBtn" title="Enable/Disable Video">
        <i class="ri-camera-line ag-icon-camera" />
        <i class="ri-camera-off-line ag-icon-camera-off" />
      </span>
      <span v-if="attendeeMode !== 'audience'" @click="handleMic" class="ag-btn audioControlBtn" title="Enable/Disable Audio">
        <i class="ri-mic-2-fill ag-icon-mic"></i>
        <i class="ri-mic-off-line ag-icon-mic-off"></i>
      </span>
      <span @click="switchDisplay" class="ag-btn displayModeBtn" :class="{ disabled: streamList.length > 4 }" title="Switch Display Mode">
        <i class="ri-tv-2-line ag-icon-switch-display"></i>
      </span>
      <span class="ag-btn disableRemoteBtn" :class="{ disabled: streamList.length > 4 || displayMode !== 'pip' }" @click="hideRemote" title="Hide Remote Stream">
...
export default {
  ...
  props: ['transcode', 'attendeeMode', 'videoProfile', 'channel', 'baseMode', 'appId', 'uid'],
  ...
  methods: {
    ...
    switchDisplay(e) {
      if (e.currentTarget.classList.contains('disabled') || this.streamList.length <= 1) {
        return
      }
